...
TICKER_MAP = {
    ...
    "جي بي كورب": "AUTO",
    "إعمار مصر": "EMFD",
    "مينا للاستثمار السياحي والعقاري": "MENA",
    "العامة لمنتجات الخزف والصيني": "PRCL",
    "جنوب الوادى للأسمنت": "SVCE",
    "الدولية للمحاصيل الزراعية": "IFAP",
    "العربية للخزف سيراميكا": "CERA",
    "العز للسيراميك والبورسلين": "ECAP",
    "العربية لحليج الأقطان": "ACGC",
    "مجموعة عامر القابضة": "AMER",
    "النصر للملابس والمنسوجات": "KABO",
    "المطورون العرب القابضة": "ARAB",
    "طاقة عربية ش.م.م": "TAQA",
    "العبوات الطبية": "MEPA",
    "المصرف المتحد": "UBEE",
...
if GEMINI_API_KEY:
    ...
    # قائمة النماذج الأكثر استقراراً مع نظام احتياطي
    MODEL_LIST = ["gemini-1.5-flash-latest", "gemini-1.5-pro-latest", "gemini-2.5-flash", "gemini-pro"]
    for m in MODEL_LIST:
        try:
...
async def analyze_news_with_gemini(title, description, tickers, fair_value_data):
    if not model:
        return "⚠️ نموذج الذكاء الاصطناعي غير متاح حالياً. سيتم إرسال الخبر مع بيانات القيمة العادلة فقط."

    # محاولة التحليل 3 مرات قبل الفشل
...
2.  **القيمة العادلة (Fair Value):** بناءً على البيانات المتاحة، ادمج القيمة العادلة المذكورة أعلاه في تحليلك وكيف يؤثر الخبر عليها.
...
**الأسلوب:** احترافي، موجز، قائم على الأرقام."""
            ...
            # أي خطأ آخر غير APIError
            return f"⚠️ خطأ في التحليل: {str(e)[:80]}. تم إرسال الخبر مع بيانات القيمة العادلة فقط."

    # في حال فشل جميع المحاولات
    return "⚠️ فشل التحليل بعد عدة محاولات. تم إرسال الخبر مع بيانات القيمة العادلة فقط."
...
**الأسلوب:** احترافي، موجز، قائم على الأرقام."""
    ...
    try:
        ...
        return f"⚠️ خطأ في التحليل: {str(e)[:80]}. تم إرسال الخبر بدون تحليل متعمق."
...
async def main():
    ...
    print(f"\n🤖 EGX News Bot v5.0 Started | Model: {selected_model_name or 'N/A'} | Tracked: {len(sent_db)}")
