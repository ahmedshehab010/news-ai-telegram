...
# هذا القاموس هو قلب نظام التكويد. يمكن توسيعه بسهولة.
TICKER_MAP = {
    ...
    "جي بي كورب": "AUTO",
    "إعمار مصر": "EMFD",
    "جورميه": "Gourmet_IPO" # مثال لسهم غير مدرج بعد
}
...
async def process_and_send(bot, title, description, link, sent_db):
    ...
    fair_value_data = get_fair_value_data(tickers) # جلب البيانات قبل التحليل
...
def format_fair_value_for_prompt(fair_value_data):
    ...
    fair_value_section = ""
    if fair_value_data: # fair_value_data تم جلبه بالفعل في الخطوة 4
...
async def main():
    ...
    load_fair_values() # تحميل بيانات القيمة العادلة عند بدء التشغيل
