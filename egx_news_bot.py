...
def smart_fallback_analysis(title, fv_data):
    ...
    # كلمات إيجابية
    positive_words = ['ارتفاع', 'نمو', 'زيادة', 'أرباح', 'توسع', 'شراكة', 'استحواذ', 'اكتتاب', 'توزيع كوبون']
    # كلمات سلبية
    ...
    number_text = f"القيمة: {numbers[0]} {title[title.find(numbers[0]):title.find(numbers[0])+30]}" if numbers else "بدون أرقام محددة"
    ...
    if fv_data:
        ...
        if upside > 15:
            valuation = "مقوم بأقل من قيمته - فرصة شراء"
            target = fv * 0.95  # قريب من القيمة العادلة
            decision = f"شراء تدريجي | الهدف {target:.2f} ج.م"
            confidence = 7
        elif upside < -10:
            valuation = "مقوم بأعلى من قيمته - حذر"
            target = fv * 1.05
            decision = f"بيع جزئي | الهدف {target:.2f} ج.م"
            confidence = 6
        else:
            valuation = "تقييم عادل - متابعة"
            target = (curr_price + fv) / 2
            decision = f"احتفاظ | الهدف {target:.2f} ج.م"
            confidence = 5
    else:
        valuation = "تحليل عام"
        decision = "متابعة | انتظار بيانات إضافية"
        confidence = 4
    
    # تحديد الاتجاه
    if positive_count > negative_count:
        direction = "إيجابي - يدعم السهم"
        impact = f"الخبر إيجابي ويعزز من {['التوسع', 'النمو', 'الأرباح'][positive_count % 3]}"
    elif negative_count > positive_count:
        direction = "سلبي - ضغط على السهم"
        impact = "الخبر قد يضغط على الأداء قصير المدى"
    else:
        direction = "محايد - متابعة"
        impact = "الخبر ذو تأثير محدود على المدى القصير"
...
5. التوصية: [شراء/بيع/احتفاظ والسعر]"""
    ...
    try:
        ...
        if len(analysis['impact']) < 10 or not analysis['recommendation']:
            print("⚠️ Gemini response weak, using fallback")
            return smart_fallback_analysis(title, fv_data)
        
        return analysis
        
    except Exception as e:
        print(f"⚠️ Gemini failed: {str(e)[:50]}, using fallback")
        return smart_fallback_analysis(title, fv_data)
...
async def main():
    ...
    print(f"\n🤖 EGX Bot v9.0 - Smart Fallback")
    print(f"📊 Gemini: {model_name or 'Disabled - using fallback'}")
    ...
    while True:
        try:
            ...
            print(f"✅ Done. DB: {len(db)}")
            await asyncio.sleep(90)  # 1.5 دقيقة
