...
def format_fair_value_for_prompt(fair_value_data):
    ...
    for ticker, data in fair_value_data.items():
        ...
        fv = f"{data.get("fair_value"):.2f}" if data.get("fair_value") is not None else "N/A"
        upside = f"{data.get("upside_percent"):.1f}%" if data.get("upside_percent") is not None else "N/A"
...
async def process_and_send(bot, title, description, link, sent_db):
    ...
    for ticker_tag in tickers:
        ...
        if ticker in fair_value_data:
            ...
            company_name = data.get("company_names", [ticker])[0]
            current_price = f"{data.get("current_price"):.2f}" if data.get("current_price") is not None else "N/A"
            fv = f"{data.get("fair_value"):.2f}" if data.get("fair_value") is not None else "N/A"
...
async def main():
    ...
    # await fetch_pulse(bot, sent_db) # Temporarily disabled as per user's focus on RSS and Excel data
